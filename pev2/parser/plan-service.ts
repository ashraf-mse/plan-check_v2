...
export class PlanService {
  ...
  private getPlanContent(value: any): IPlanContent {
    ...
    // Support lowercase 'plan' key
    if (value.plan) {
      return { ...value, Plan: value.plan }
    }

    // Support naked plan object (if it contains Node Type)
    if (value["Node Type"]) {
      return { Plan: value }
    }

    throw new Error("Invalid plan: JSON must contain a 'Plan' or 'plan' object, or be the plan itself.")
  }
